                        ...
                        for glyph_run in line.glyph_runs() {
                            ...
                            if let Ok(font_ref) = FontRef::from_index(font_ref.data, font.index()) {
                                ...
                                for glyph in glyph_run.glyphs() {
                                    if let Some(text_fragment) =
                                        gp.get(glyph.id, Some(&style.brush.0))
                                    {
                                        let gx = x + glyph.x;
                                        let gy = y - glyph.y;
                                        let xform = Affine::translate((gx as f64, gy as f64))
                                            * Affine::scale_non_uniform(1.0, -1.0);
                                        fragment.append(&text_fragment, Some(transform * xform));
                                    }
                                    x += glyph.advance;
                                }
                            }
...
impl<'a> Renderer for RenderEngine<'a> {
    fn render(&mut self, mut command_lists: Vec<Vec<RenderCommand>>, debug: &mut DebugLayer) {
